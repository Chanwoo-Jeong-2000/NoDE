...
from torch.nn import Embedding  # NoDE (Ours)
...
class LayerGCN(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        self.beta = Embedding(self.n_items, 1)  # NoDE (Ours)
        nn.init.xavier_uniform_(self.beta.weight)  # NoDE (Ours)
    ...
    def pre_epoch_processing(self):
        ...
        if self.pruning_random:
            ...
            keep_idx = torch.multinomial(self.edge_values, keep_len).to(self.device)  # ensure keep_idx is on the same device

        self.pruning_random = True ^ self.pruning_random
        
        # Ensure self.edge_indices is on the same device as keep_idx
    ...
    def forward(self):
        ...
        item_all_embeddings = F.normalize(item_all_embeddings, p=2, dim=1) * 2.5  # NoDE (Ours)
    ...
    def bpr_loss(self, u_embeddings, i_embeddings, user, pos_item, neg_item):
        ...
        # calculate BPR Loss
        pos_scores = torch.mul(u_embeddings, posi_embeddings).sum(dim=1) + self.beta.weight[pos_item]  # NoDE (Ours)
        neg_scores = torch.mul(u_embeddings, negi_embeddings).sum(dim=1) + self.beta.weight[neg_item]  # NoDE (Ours)
    ...
    def full_sort_predict(self, interaction):
        ...
        beta = self.beta.weight  # NoDE (Ours)
        beta_expand = beta.t().expand(user.shape[0], -1)  # NoDE (Ours)

        # dot with all item embedding to accelerate
        scores = torch.matmul(u_embeddings, restore_item_e.transpose(0, 1)) + beta_expand  # NoDE (Ours)
        return scores
