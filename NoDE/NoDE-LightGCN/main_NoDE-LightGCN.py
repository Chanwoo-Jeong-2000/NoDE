...
from model import NoDE_LightGCN
...
parser.add_argument('--scaling_factor', type=float, default=2.5)
...
print('##############################')
print('This model is NoDE-LightGCN!')
...
def train():
    ...
    for index in train_loader: #for index in tqdm(train_loader):
        ...
        optimizer.zero_grad()
        pos_rank, neg_rank = model(data.edge_index, edge_label_index, num_users=num_users, num_items=num_items, scaling_factor=args.scaling_factor)
...
@torch.no_grad()
def test(ks: list):
    emb = model.get_embedding(data.edge_index, num_users=num_users, num_items=num_items, scaling_factor=args.scaling_factor)
    user_emb, item_emb = emb[:num_users], emb[num_users:]

    beta = model.beta.weight[num_users:] # ours: apply beta
    beta_expand = beta.t().expand(num_users, -1) # ours: apply beta
    ...
    for k in ks:
        ...
        for start in range(0, num_users, batch_size):
            ...
            logits = user_emb[start:end] @ item_emb.t() + beta_expand[start:end] # ours: apply beta
...
path_name = 'result/'+args.dataset+'/NoDE-LightGCN_'+args.dataset+args.path_name+'.pt'
...
if not early_stopping.early_stop:
    path_name = 'result/'+args.dataset+'/NoDE-LightGCN_'+args.dataset+'_not_early_stop.pt'
