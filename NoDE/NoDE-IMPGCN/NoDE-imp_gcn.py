...
class IMP_GCN(GeneralRecommender):
    ...
    def forward(self):
        ...
        i_g_embeddings = F.normalize(i_g_embeddings, p=2, dim=1) * 2.5  # NoDE (Ours)

        return u_g_embeddings, i_g_embeddings
        #return u_g_embeddings, i_g_embeddings, A_fold_hat_group_filter, user_group_embeddings_sum

    def bpr_loss(self, users, pos_items, neg_items, users_pre, pos_items_pre, neg_items_pre, pos_item, neg_item):  # NoDE (Ours)
        pos_scores = torch.sum(torch.mul(users, pos_items), dim=1) + self.beta.weight[pos_item]  # NoDE (Ours)
        neg_scores = torch.sum(torch.mul(users, neg_items), dim=1) + self.beta.weight[neg_item]  # NoDE (Ours)
    ...
    def full_sort_predict(self, interaction):
        ...
        scores = torch.matmul(u_embeddings, self.restore_item_e.transpose(0, 1)) + beta_expand  # NoDE (Ours)
